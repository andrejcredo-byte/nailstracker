...
export const Heatmap: React.FC = () => {
  ...
  const mySessions = useMemo(() => {
    if (!user) return [];
    return data.sessions.filter(s => s.telegram_id === user.id);
  }, [data.sessions, user]);

  const streak = useMemo(() => {
    if (!user) return 0;
    return calculateStreak(data.sessions, user.id);
  }, [data.sessions, user]);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Последние 30 дней</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-2xl font-black text-emerald-500 tracking-tighter">{streak}</div>
